...
export default function PregnancyCalculator() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) {
        const { mode: m, date, weekDirect } = JSON.parse(saved)
        setMode(m)
        if (m === 'week' && weekDirect) {
          setWeekInput(String(weekDirect))
          setWeek(weekDirect)
        } else if (date) {
          setDateInput(date)
  ...
  function handleWeekChange(value: string) {
    ...
    if (!value || isNaN(num) || num < 1 || num > 42) { setWeek(null); return }
    setWeek(num)
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify({ mode: 'week', weekDirect: num })) } catch {}
  }
      ...
      <div className="px-5 pt-5 pb-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex text-xs bg-gray-100 rounded-lg p-0.5">
            {([['due', '예정일'], ['lmp', '생리일'], ['week', '주차 입력']] as [Mode, string][]).map(([m, label]) => (
              <button
                key={m}
                onClick={() => handleModeChange(m)}
                className={`px-2.5 py-1 rounded-md transition-colors ${mode === m ? 'bg-white text-gray-700 shadow-sm font-medium' : 'text-gray-400'}`}
              >
                {label}
              </button>
            ))}
        ...
        {mode === 'week' ? (
          <div className="flex items-center gap-3">
            <input
              ...
              placeholder="예: 24"
              className="w-32 text-sm border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none focus:border-rose-300 text-gray-700"
            />
            <span className="text-sm text-gray-400">주차 (1~42)</span>
          </div>
        ) : (
          <input
            type="date"
